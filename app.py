...
def create_pdf_report(data, scenarios, wacc, z_score):
    pdf = InvestmentMemo()
    pdf.add_page()
    ...
    pdf.multi_cell(0, 10, f"Current Market Price: ${data['price']:.2f}\n"
                          f"Implied Intrinsic Value (Base Case): ${scenarios['Base']:.2f}\n"
                          f"WACC Used: {wacc:.2%}\n"
    ...
    for k, v in scenarios.items():
        pdf.cell(0, 10, f"{k} Case Target: ${v:.2f}", 0, 1)
        
    pdf.ln(10)
    pdf.set_font("Arial", 'I', 8)
    pdf.multi_cell(0, 5, "Disclaimer: This report is generated by DEBIN CAPITAL's automated algorithm. It does not constitute financial advice.")
...
def process_uploaded_data(df_upload):
    try:
        ...
        return {
            ...
            "net_income": float(row.get('Net Income', 0)), "equity": float(row.get('Equity', 1000)),
    ...
    if 'data' in st.session_state:
        ...
        if not d['hist'].empty:
            ...
            fig.update_layout(height=600, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', 
                              font=dict(color='#dcd6cc'), xaxis=dict(showgrid=False, rangeslider=dict(visible=False)), 
            ...
            if 'Volume' in hist.columns:
                ...
                vol.update_layout(height=150, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
                                  font=dict(color='#888'), margin=dict(t=0, b=0), yaxis=dict(showgrid=False))
...
# 7. HEALTH CHECK & DUPONT (Feature #2 Integrated)
    ...
    if 'data' in st.session_state:
        ...
        try:
            ...
            d1.metric("Net Profit Margin", f"{net_margin:.1%}", help="Efficiency")
            d2.metric("Asset Turnover", f"{asset_turnover:.2f}x", help="Asset Use")
            d3.metric("Financial Leverage", f"{fin_leverage:.2f}x", help="Debt Multiplier")
