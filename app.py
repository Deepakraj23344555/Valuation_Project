...
def get_market_data(ticker):
    try:
        ...
        info = stock.info
        hist = stock.history(period="2y") # Fetch 2 years for SMA 200
        try:
...
with st.sidebar:
    ...
    st.caption("Professional Edition v6.0")
    st.markdown("---")
    ...
    if ticker:
        with st.spinner(f"Fetching live data for {ticker}..."):
            ...
            if mkt_data:
                ...
                st.markdown("### Technical View & News")
                ...
                fig_candle.update_layout(
                    title=f"{mkt_data['name']} - Price vs SMA 50/200",
                ...
                try:
                    ...
                    if news:
                        for item in news[:3]: # Show top 3 news
                            ...
                            if sentiment > 0.1: icon = "🟢"
                            elif sentiment < -0.1: icon = "🔴"
                            else: icon = "⚪"
                            
                            st.markdown(f"{icon} **[{title}]({link})**")
                            st.caption(f"Published: {pub_time} | Source: {publisher}")
                            st.divider()
                    else:
                        st.info("No news feed available for this ticker.")
                except Exception as e:
                    st.info(f"Could not fetch news: {e}")
